...
<c:set value="${empty prefix ? '' : prefix.concat('-')}" var="prefix"/>
<div class="date-range-widget">
<ul class="key-value-list">
    <li>
        <div class="li-key"><label ${required ? 'class="required-field"' : ''} for="${prefix}date-range">Date Range</label></div>
        <div class="li-value">
            <select id="${prefix}date-range" class="date-range-select ${datetime ? 'datetime-range' : 'date-range'} ${sevenAmOffset ? ' seven-am-offset' : ' midnight-offset'}">
...
<ul class="custom-date-range-list key-value-list" style="display: none;">
    <li>
        <div class="li-key">
            <label ${required ? 'class="required-field"' : ''} for="${prefix}start" title="Inclusive (Closed)">Start
                Date</label>
            <div class="date-note">(Inclusive)</div>
        </div>
        <div class="li-value">
            <input type="text" class="start-input ${datetime ? 'datetime' : 'date'}-input" id="${prefix}start" name="start" autocomplete="off"
                   placeholder="DD-MMM-YYYY ${datetime ? 'hh:mm' : ''}" value="${fn:escapeXml(param.start)}"/>
        </div>
    </li>
    <li>
        <div class="li-key">
            <label ${required ? 'class="required-field"' : ''} for="${prefix}end" title="Exclusive (Open)">End Date</label>
            <div class="date-note">(Exclusive)</div>
        </div>
        <div class="li-value">
            <input type="text" class="end-input ${datetime ? 'datetime' : 'date'}-input" id="${prefix}end" name="end" autocomplete="off"
