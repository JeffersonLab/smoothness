...
public final class TimeUtil {
  ...
  /**
   * WARNING WARNING WARNING : Use calculateCrewChiefShiftEndDayAndHour instead if you are providing
   * the startDayAndHour. This method is only if the "day" parameter is the actual day that pairs
   * with the shift name. The gotcha is with OWL shift where it technically starts the day before!
   *
   * @param day The day
   * @param shift The shift
   * @return The end date
  ...
  /**
   * A really complex way to say give me "now". This is complex due to Crew Chief's days starting at
   * 23 the previous day. This means if the hour is 23 then actually return tomorrow.
   *
   * <p>Wait, there is more complication:
   *
   * <p>We take Date as a parameter so we can avoid race condition when using this method plus
   * TimeUtil.calculateCrewChiefShiftType. If the calls to new Date() are independent in each then
   * we can get into trouble if boundaries are crossed. For example: if getCurrentCrewChiefShiftDay
   * uses 23:59 and new Date() uses 00:00. Another example: 22:59 vs 23:00.
   *
  ...
  /**
   * Smoothness template handles a set of human-friendly labeled ranges, and this method checks if a
   * given start and end Date range matches one of the named ranges. If not, "custom" is returned.
   * This is useful because instead of always showing users some explicit verbose date range, using
   * familiar named ranges is convenient when possible. For example, running a report on "Last Year"
   * is concise.
   *
   * @param start The start Date
   * @param end The end Date
   ...
   * @return The human-friendly range label, or 'custom' if not in the set of smoothness-honored
   *     ranges.
   */
  public static String encodeRange(
      Date start, Date end, boolean sevenAmAdjusted, DateRange currentRun, DateRange previousRun) {
    Calendar c = Calendar.getInstance();
    Date now = new Date();
    ...
    if (end.getTime() == currentShiftEnd.getTime()
      ...
      range = "1day";
    } else if (end.getTime() == currentWeekEnd.getTime()
        && start.getTime() == currentWeekStart.getTime()) {
      range = "0week";
    } else if (end.getTime() == previousWeekEnd.getTime()
        && start.getTime() == previousWeekStart.getTime()) {
      range = "1week";
    } else if (end.getTime() == currentMonthEnd.getTime()
        && start.getTime() == currentMonthStart.getTime()) {
      range = "0month";
    } else if (end.getTime() == previousMonthEnd.getTime()
        && start.getTime() == previousMonthStart.getTime()) {
      range = "1month";
    } else if (end.getTime() == currentYearEnd.getTime()
        && start.getTime() == currentYearStart.getTime()) {
      range = "0year";
    } else if (end.getTime() == previousYearEnd.getTime()
        && start.getTime() == previousYearStart.getTime()) {
      range = "1year";
    } else if (end.getTime() == currentFiscalYearEnd.getTime()
        && start.getTime() == currentFiscalYearStart.getTime()) {
      range = "0fiscalyear";
    } else if (end.getTime() == previousFiscalYearEnd.getTime()
        && start.getTime() == previousFiscalYearStart.getTime()) {
      range = "1fiscalyear";
    } else if (end.getTime() == previousFiscalYearQ1End.getTime()
        && start.getTime() == previousFiscalYearQ1Start.getTime()) {
      range = "1fiscalyearq1";
    } else if (end.getTime() == previousFiscalYearQ2End.getTime()
        && start.getTime() == previousFiscalYearQ2Start.getTime()) {
      range = "1fiscalyearq2";
    } else if (end.getTime() == previousFiscalYearQ3End.getTime()
        && start.getTime() == previousFiscalYearQ3Start.getTime()) {
      range = "1fiscalyearq3";
    } else if (end.getTime() == previousFiscalYearQ4End.getTime()
        && start.getTime() == previousFiscalYearQ4Start.getTime()) {
      range = "1fiscalyearq4";
    } else if (end.getTime() == currentFiscalYearQ1End.getTime()
        && start.getTime() == currentFiscalYearQ1Start.getTime()) {
      range = "0fiscalyearq1";
    } else if (end.getTime() == currentFiscalYearQ2End.getTime()
        && start.getTime() == currentFiscalYearQ2Start.getTime()) {
      range = "0fiscalyearq2";
    } else if (end.getTime() == currentFiscalYearQ3End.getTime()
        && start.getTime() == currentFiscalYearQ3Start.getTime()) {
      range = "0fiscalyearq3";
    } else if (end.getTime() == currentFiscalYearQ4End.getTime()
        && start.getTime() == currentFiscalYearQ4Start.getTime()) {
      range = "0fiscalyearq4";
    } else if (currentRun != null
        && currentRun.getEnd().getTime() == end.getTime()
        && currentRun.getStart().getTime() == start.getTime()) {
      range = "0run";
    } else if (previousRun != null
        && previousRun.getEnd().getTime() == end.getTime()
        && previousRun.getStart().getTime() == start.getTime()) {
      range = "1run";
  ...
  /**
   * Return a formatted String representation of a date range using "smart human" shortcuts.
   * Specifically, any time with zero minutes and zero hours are don't show hours and minutes
   * (they're implied). If the start and end are in the same month or same year, then the month or
   * year respectively is not shown more than once (it's implied).
   *
   * @param start The start Date
   * @param end The end Date
    ...
    if (sHourFormat.length() > 0 || eHourFormat.length() > 0) {
      ...
      if (!oneDaySpecialCase) {
        ...
        if (!oneMonthSpecialCase && firstDayOfMonth) {
          ...
          oneYearSpecialCase =
              (special.get(Calendar.MONTH) == Calendar.JANUARY && special.getTime().equals(end));

          if (!oneYearSpecialCase) {
            fiscalYearSpecialCase =
                (special.get(Calendar.MONTH) == Calendar.OCTOBER && special.getTime().equals(end));
          }
        }
      }
    }
